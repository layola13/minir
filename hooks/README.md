# Mimir Hooks — Auto-Save for Terminal AI Tools

These hook scripts make Mimir save automatically. No manual "save" commands needed.
...
```json
{
  "hooks": {
    "Stop": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "/absolute/path/to/hooks/mimir_save_hook.sh",
        "timeout": 30
      }]
    }],
    "PreCompact": [{
      "hooks": [{
        "type": "command",
        "command": "/absolute/path/to/hooks/mimir_precompact_hook.sh",
        "timeout": 30
      }]
    }]
  }
...
```bash
chmod +x hooks/mimir_save_hook.sh hooks/mimir_precompact_hook.sh
```
...
Edit `mimir_save_hook.sh` to change:

- **`SAVE_INTERVAL=15`** — How many human messages between saves. Lower = more frequent saves, higher = less interruption.
- **`STATE_DIR`** — Where hook state is stored (defaults to `~/.mimir/hook_state/`)
- **`MEMPAL_DIR`** — Optional. Set to a conversations directory to auto-run `mimir mine <dir>` on each save trigger. Leave blank (default) to let the AI handle saving via the block reason message.
...
```bash
mimir mine <dir>               # Mine all files in a directory
mimir mine <dir> --mode convos # Mine conversation transcripts only
```
