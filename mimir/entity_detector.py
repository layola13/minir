...
Used by mimir init before mining begins.
...
PROJECT_VERB_PATTERNS = [
    ...
    r"\b{name}\s+v\d+\b",  # Mimir v2
    r"\b{name}\.py\b",  # mimir.py
    r"\b{name}-core\b",  # mimir-core (hyphen only, not underscore)
    r"\b{name}-local\b",
    r"\bimport\s+{name}\b",
